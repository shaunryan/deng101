...
// MAGIC %md 
// MAGIC #Load Weather - Multi Field Json Validation and Shred
...
val schemaColumn1 = "Weather"
val schemaColumn2 = "Data"
val sparkSchema1 = pipeline.source.getSparkSchema(schemaColumn1)
val sparkSchema2 = pipeline.source.getSparkSchema(schemaColumn2)
...
// standard etl columns start
  ...
  .withColumn(s"InvalidMessage{schemaColumn1}", expr(s"CASE WHEN ${schemaColumn1}InValidMsgs = '' THEN NULL ELSE ${schemaColumn1}InvalidMsgs END"))
  .withColumn(s"IsValid{schemaColumn1}", expr(s"if(${schemaColumn1}InValid>0, false, true)"))
  .withColumn(s"Shredded${schemaColumn1}", from_json(col(schemaColumn), sparkSchema))

  .withColumn(s"InvalidMessage{schemaColumn2}", expr(s"CASE WHEN ${schemaColumn2}InValidMsgs = '' THEN NULL ELSE ${schemaColumn2}InvalidMsgs END"))
  .withColumn(s"IsValid{schemaColumn2}", expr(s"if(${schemaColumn2}InValid>0, false, true)"))
  .withColumn(s"Shredded${schemaColumn2}", from_json(col(schemaColumn), sparkSchema))
